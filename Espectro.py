...
y = 0               # pi*(massa_adjunta**2)/(N*(g**2))      

point_size = 2      # tamanho dos pontos no gráfico
...
def matriz_hamiltoniana_T(set_states_list):
    ...
    matrix_A = np.zeros((lenght,lenght), dtype=np.float32)
...
lightest_hadron_ferm = []
lightest_hadron_boso = []
inverse_k_axis_ferm = []
inverse_k_axis_boso = []
...
while(k <= K_max):
    ...
    eigenvalues.sort()
    if(k % 2):
        lightest_hadron_ferm.append(eigenvalues[0])
        inverse_k_axis_ferm.append(1/k)
    else:
        lightest_hadron_boso.append(eigenvalues[0])
        inverse_k_axis_boso.append(1/k)
    ...
    if(counter < 2):
        if(k % 2 != 0):     # fermions
            matplotlib.pyplot.plot(x_axis, eigenvalues, marker='o',color='red', linestyle='', label = 'Férmions', markersize=point_size)
        else:               # bosons
            matplotlib.pyplot.plot(x_axis, eigenvalues, marker='o',color='blue', linestyle='', label = 'Bósons', markersize=point_size)
    else:
        if(k % 2 != 0):     # fermions
            matplotlib.pyplot.plot(x_axis, eigenvalues, marker='o',color='red', linestyle='', markersize=point_size)
        else:               # bosons
            matplotlib.pyplot.plot(x_axis, eigenvalues, marker='o',color='blue', linestyle='', markersize=point_size)
...
# lightest fermions states

m = len(lightest_hadron_ferm)      # número de pontos / amostras
m1 = m - 1       # imax, kmax

gjl_matrix(gjl, m1, n1, inverse_k_axis_ferm)
gamma_column(gammal, m1, n1, inverse_k_axis_ferm, lightest_hadron_ferm)
cj = np.linalg.solve(gjl, gammal) 

# lightest bosons states

m = len(lightest_hadron_boso)      # número de pontos / amostras
m1 = m - 1       # imax, kmax

gjl = np.zeros((n,n))
gammal = np.zeros(n)
gjl_matrix(gjl, m1, n1, inverse_k_axis_boso)
gamma_column(gammal, m1, n1, inverse_k_axis_boso, lightest_hadron_boso)
...
print("Extrapolação: ")
print("Fermion mais leve: ", f(0,n1,cj))
print("Boson mais leve: ", f(0,n1,dj))
print(" ")
fim = time.time()
...
matplotlib.pyplot.ylim([0,53])     #  [0,113]  ou [0,70]
